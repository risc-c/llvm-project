...
# RUN: llvm-mc -triple=riscc-none-elf -mcpu=nano -filetype=obj < %s -o %t
# RUN: llvm-objdump -d %t | FileCheck %s --check-prefix=DIS
# RUN: llvm-objdump -d --mcpu=min %t | FileCheck %s --check-prefix=OVERRIDE
